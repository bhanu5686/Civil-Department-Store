...
if(isset($_POST['last_video_id']))
{
	...
	$sqls="SELECT * FROM items WHERE id > :item_id AND price BETWEEN :left AND :right LIMIT 9";
	$results = $con->prepare($sqls);
	$results->execute(array('item_id'=>$item,'left'=>$left,'right'=>$right));
}
else
{
	$sqls="SELECT * FROM items WHERE price BETWEEN :left AND :right LIMIT 9";
	$results = $con->prepare($sqls);
	$results->execute(array('left'=>$left,'right'=>$right));
}
...
{  
     ...
     {  
		  ...
		  {
			 $output .= ' <div class="col-lg-4 col-sm-6" id="tabledata">
				...
				else
				{
					$output .= '<div class="col-lg-4 col-sm-6" id="tabledata">
								...
								<div class="pi-pic">	
									<a href="product.php?id='. $row['id'] .'"><img src="img/'. $row['path'] .'" alt="image"></a>	
									<div class="pi-links">
							<a href="cart_add.php?id='. $row['id'] .'&cid='. mt_rand(100,999) .'" class="add-card"><i class="flaticon-bag"></i><span>ADD TO CART</span></a>
							<a href="#" class="wishlist-btn"><i class="flaticon-heart"></i></a>
							</div>
							</div>
						<div class="pi-text">
